    ...
    allPlayers
      .filter(
        (p) =>
          !xi.includes(p) &&
          (searchTerms[index] === "" ||
            p.toLowerCase().includes(searchTerms[index].toLowerCase()))
      )
      .slice(0, 20);
...
export default function MatchForm() {
  ...
  const handlePredict = async () => {
    ...
    if (xi1.filter(Boolean).length !== 11 || xi2.filter(Boolean).length !== 11) {
      alert("Select exactly 11 players for both teams");
      return;
    }
